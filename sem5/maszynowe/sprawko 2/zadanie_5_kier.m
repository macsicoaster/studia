...
%% idk czy chcemy to na dwa sposoby robić
%
...
% siec_newff.trainParam.epochs = 1000; % zadana liczba epok 
% siec_newff.trainParam.goal = 0; %  docelowy błąd 

% % wyznacznie x

% out_data_train = [wartosci_ukladu(1:length(wartosci_ukladu)-2,1)];
% siec_newffx = train(siec_newff,in_data_train',out_data_train');
% x_ff = sim(siec_newffx, in_data_train');
% xt_ff = sim(siec_newffx,in_data_test');


% % wyznaczanie y 
% out_data_train = [wartosci_ukladu(1:length(wartosci_ukladu)-2,2)];
% siec_neffy  = train(siec_newff,in_data_train',out_data_train');
% y_ff = sim(siec_neffy, in_data_train');
% yt_ff = sim(siec_neffy,in_data_test');

% % wyznaczanie z
% out_data_train = [wartosci_ukladu(1:length(wartosci_ukladu)-2,3)];
% siec_newffz = train(siec_newff,in_data_train',out_data_train');
% z_ff = sim(siec_newffz, in_data_train');
% zt_ff = sim(siec_newffz,in_data_test');

% figure(3)
% title('each variable with diferent network')
% subplot(2,3,1);grid;box;hold;
% plot(x);
% plot(x_ff);
% title('x value');
% legend('expected', 'sim on trained data');

% subplot(2,3,2);grid;box;hold;
% plot(y);
% plot(y_ff);
% title('y value');
% legend('expected', 'sim on trained data');

% subplot(2,3,3);grid;box;hold;
% plot(z);
% plot(z_ff);
% title('z value');
% legend('expected', 'sim on trained data');

% subplot(2,3,4);grid;box;hold;
% plot(x);
% plot(xt_ff);
% title('x value');
% legend('expected', 'sim on test data');

% subplot(2,3,5);grid;box;hold;
% plot(y);
% plot(yt_ff);
% title('y value');
% legend('expected', 'sim on test data');

% subplot(2,3,6);grid;box;hold;
% plot(z);
% plot(zt_ff);
% title('z value');
% legend('expected', 'sim on test data');
...
% figure(4);grid;box;hold;
% plot3(x,y,z);
% plot3(x_ff,y_ff,z_ff)
% plot3(xt_ff,yt_ff,zt_ff);
% legend('expected', 'simulated on data', 'simulated on test data');
...
plot(wynikxyz(2,:));
plot(wynikxyz(3,:));
title(['newff training data , mse: ' num2str(tr_mse_ff)])

subplot(1,3,3);grid;hold;box;
plot(wyniki_test(1,:));
plot(wyniki_test(2,:));
plot(wyniki_test(3,:));
title(['newff test data , mse: ' num2str(test_mse_ff)])
